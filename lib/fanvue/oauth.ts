...
function getFanvueOAuthCredentials() {
...
function fanvueTokenRequestHeaders(clientId: string, clientSecret: string): HeadersInit {
  return {
...
export async function exchangeFanvueCodeForToken(params: {
  ...
  const { clientId, clientSecret } = getFanvueOAuthCredentials()
  ...
  const response = await fetch(`${FANVUE_AUTH_BASE_URL}/oauth2/token`, {
    method: "POST",
    headers: fanvueTokenRequestHeaders(clientId, clientSecret),
    body,
  })
