  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; } body { margin: 0; background: var(--color-background-primary); color: var(--color-text-primary); font-family: var(--font-sans); } button { font: inherit; }
      #app { padding: 10px; } .card { background: var(--color-background-secondary); border: 1px solid var(--color-border-primary); border-radius: var(--border-radius-lg); overflow: hidden; } .media-wrap { background: #252525; display: flex; align-items: center; justify-content: center; min-height: 160px; } .media { display: block; width: 100%; } .media.media-image { height: auto; max-height: min(560px, 72vh); object-fit: contain; } .media.media-video { max-height: min(560px, 72vh); object-fit: contain; } video.media { aspect-ratio: 16 / 9; width: 100%; } audio.media { margin: 12px; width: calc(100% - 24px); } .body { display: grid; gap: 8px; padding: 13px 14px 14px; } .eyebrow { align-items: center; color: var(--color-text-secondary); display: flex; font-size: 12px; gap: 7px; } .title { font-size: 15px; font-weight: 620; letter-spacing: -.01em; } .prompt { color: var(--color-text-secondary); font-size: 13px; line-height: 1.35; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; } .detail { color: var(--color-text-secondary); font-size: 12px; } .link-row { display: flex; flex-wrap: wrap; gap: 10px; } .pending-art { align-items: center; background: linear-gradient(120deg, #222, #343434, #222); background-size: 200% 100%; display: flex; height: 190px; justify-content: center; animation: shimmer 1.8s ease-in-out infinite; } .pending-orb { background: var(--color-accent); border-radius: 50%; height: 20px; opacity: .8; width: 20px; animation: breathe 1.3s ease-in-out infinite; } .error-art { align-items: center; background: #2b2020; color: #d8a6a6; display: flex; font-size: 13px; justify-content: center; min-height: 150px; padding: 24px; text-align: center; } .action { background: transparent; border: 0; color: var(--color-text-primary); cursor: pointer; font-size: 13px; padding: 0; text-align: left; text-decoration: underline; text-underline-offset: 3px; width: fit-content; } .dot { background: currentColor; border-radius: 50%; height: 6px; width: 6px; } .dot.live { animation: breathe 1.3s ease-in-out infinite; } @keyframes breathe { 50% { opacity: .35; transform: scale(.75); } } @keyframes shimmer { to { background-position: -200% 0; } }
    </style>
  </head>
  <body>
    ...
    <script>
      ...
      function pushModelContext(item) {
        ...
        const pageUrl = text(item.pageUrl);
        const content = [{ type: "text", text: "Generated " + kind + " is ready." }];
        if (pageUrl) {
          content.push({
            type: "resource_link",
            uri: pageUrl,
        ...
        request("ui/update-model-context", { content });
      }
      ...
      function render(result, isRefresh) {
        ...
        if (item.url && !failed) {
          ...
          let media;
          if (kind === "video") { media = document.createElement("video"); media.controls = true; media.playsInline = true; media.className = "media media-video"; media.onloadeddata = () => resize(); }
          else if (kind === "audio") { media = document.createElement("audio"); media.controls = true; media.className = "media"; media.onloadeddata = () => resize(); }
          else { media = document.createElement("img"); media.alt = text(item.prompt || "Generated image"); media.className = "media media-image"; media.onload = () => resize(); }
          media.src = item.url;
        ...
        if (item.pageUrl || item.url) {
          const links = document.createElement("div"); links.className = "link-row";
          if (item.pageUrl) {
            const openApp = document.createElement("button"); openApp.type = "button"; openApp.className = "action"; openApp.textContent = "Open in UniCan"; openApp.onclick = () => open(item.pageUrl); links.appendChild(openApp);
          }
          if (item.url && item.url !== item.pageUrl) {
            const openMedia = document.createElement("button"); openMedia.type = "button"; openMedia.className = "action"; openMedia.textContent = "Open image"; openMedia.onclick = () => open(item.url); links.appendChild(openMedia);
          }
      ...
      request("ui/initialize", { appInfo: { name: "unican-media-output", version: "1.3.0" }, appCapabilities: { availableDisplayModes: ["inline", "fullscreen"] }, protocolVersion: "2025-11-25" });
    </script>
  </body>
</html>
`.trim()
