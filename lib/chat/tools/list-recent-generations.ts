...
export function createListRecentGenerationsTool({
  ...
  return tool({
    description:
      "List the user's recent generations from chat and history. Use this when the user refers to a past render, wants to reuse or save an earlier output, asks about their last generation, or when you need a generation id before saving it as an asset. Each row has `id` (raw UUID for saveGenerationAsAsset / credits) and `mediaId` (`gen_<uuid>`) to pass as **referenceIds** on generateImage / generateVideo, or as **referenceAudioIds** on generateVideo for audio clips. For user uploads in the current thread, use **listThreadMedia** (`upl_<uuid>`) with saveGenerationAsAsset instead.",
