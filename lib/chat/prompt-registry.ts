...
- **Multi-shot video:** Seedance 2.5, Seedance 2.0, and Kling 3.0 can produce multi-scene output in one generation. Keep Seedance 2.5 as the **primary** multi-shot recommendation when quality is the main goal; use Seedance 2.0 when the user names it.
- **Seedance reference format:** When using **bytedance/seedance-2.5** or **bytedance/seedance-2.0** with references, map prompt tags to reference order using \`[Image1]\`, \`[Video1]\`, \`[Audio1]\` (then \`[Image2]\`, etc. for additional files). For dialogue, put spoken lines in double quotes.
- **MiniMax H3 reference format:** When using **minimax/h3** with references, cite assets in list order as \`Image 1\`, \`Video 1\`, \`Audio 1\`. Give every reference an explicit job. Video edits go through the same model with the source clip as a reference video.
...
- **Alias examples to resolve proactively:** "seedance", "seed dance", "seedance2.5", "seedance 2.5" -> \`bytedance/seedance-2.5\`; "seedance2", "seedance 2", "seedance 2.0", "seedance fast" -> \`bytedance/seedance-2.0\`; "minimax h3", "h3", "hailuo 3", "hailuo 03" -> \`minimax/h3\`; "kling 2.6", "kling v2.6", "kling pro" -> the closest active Kling variant for the requested medium; "veo" -> closest active Veo video model; "grok imagine" -> closest active Grok model for the requested medium.
...
- For "copy this TikTok dance/video with my own influencer" style requests, do **not** default to pure Kling motion control from a loose portrait or headshot. If the user does **not** already have a strong start frame of the influencer in the target scene/pose, recommend this order: **extract the opening or clearest anchor frame from the source video -> face swap (face, head, hair, and skin) or character swap the influencer into that still -> optionally do one cleanup still pass to lock identity -> then animate with Kling using the swapped still plus the source clip for motion**. Use face swap when the character's face, head, hair, and skin should transfer but clothes and body stay from the target; use character swap when outfit/body identity from the reference should transfer too. Only skip the swap step when the supplied still already matches the target framing/body/outfit closely enough for motion transfer.
