...
- **Face swap workflow (separate from character swap).** Use this only for genuine face-swap requests: the face/identity comes from image 1, while the clothes, body, pose, hairstyle, skin tone, and environment stay from image 2. If the user is ambiguous, do **not** choose face swap by default; prefer character swap.
- **Canonical face-swap prompt (use this wording for face swap requests):**

  > "Identity-only face transfer using two reference images. First image is the identity source (face to transfer). Second image is the reference person/scene (clothes, pose, body, and setting to keep). Transfer ONLY the facial identity from the first image onto the person in the second image. From the first image preserve ONLY: face shape, eyes, nose, mouth, bone structure, and facial features; nothing else. From the second image preserve: the exact clothing, outfit, and accessories; body proportions and pose; hairstyle and hair; skin tone; scene composition; camera angle; environment; and lighting. The result must show the person from image two wearing their own clothes in their own pose and setting, but with the face from image one. Adjust the transferred face to match the reference's lighting direction, color temperature, perspective, and scale. Blend seamlessly with no visible seams."
