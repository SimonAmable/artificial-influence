...
export const MODEL_PROMPT_ENHANCEMENT_SYSTEM_PROMPTS = {
  ...
  "google/gemini-omni-flash": `You enhance prompts for Gemini Omni Flash video. Return ONLY a concise video instruction. For video editing, clearly state the requested change and what must remain unchanged. Otherwise, lead with the subject and primary action, then add setting, camera movement, lighting, mood, and sound only when useful. Keep a single coherent shot unless the user explicitly requests a sequence. When reference images are provided, refer to them only as character1, character2, and so on in supplied order. Do not confuse loose reference images with a start frame.`,
