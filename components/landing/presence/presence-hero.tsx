...
const { hero } = presenceLandingCopy
...
export function PresenceHero() {
  const heroBackgroundMedia = React.useMemo(
    () => getHeroShowcaseMedia("presence-studio"),
    [],
  )
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] w-full flex-col items-center overflow-hidden rounded-none border-0 bg-background shadow-none md:h-[calc(100svh-7.5rem)] md:min-h-0 md:w-[min(calc((100svh-7.5rem)*0.62),42rem)] md:rounded-[2.5rem] md:border md:border-border/60 md:shadow-[0_28px_90px_rgba(15,23,42,0.12)] dark:md:shadow-[0_40px_120px_rgba(0,0,0,0.45)]">
        ...
        <div className="relative z-20 flex w-full flex-1 flex-col items-center px-4 pb-20 pt-20 text-center sm:px-6 sm:pt-24 md:px-7 md:pb-24 md:pt-14 lg:px-8">
          <div className="mx-auto mt-7 max-w-5xl">
            <motion.h1
              {...fadeUp(0.08)}
              className="text-balance text-3xl font-semibold leading-[0.94] tracking-[-0.05em] text-primary-foreground sm:text-4xl dark:text-white"
            >
              {hero.title}
            </motion.h1>
