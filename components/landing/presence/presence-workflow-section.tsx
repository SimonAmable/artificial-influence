...
import {
  presenceLandingCopy,
  presencePlatformSurfaceCards,
} from "@/lib/constants/presence-landing-content"
...
export function PresenceWorkflowSection() {
  const sectionRef = React.useRef<HTMLElement>(null)
  const platformSurfaceCards = presencePlatformSurfaceCards
  ...
  }, [slideCount, platformSurfaceCards])

  const handleUserTabChange = React.useCallback(
    (nextTab: string) => {
      markInteracted()
      setTab(nextTab)
    },
    ...
    <section ref={sectionRef} className="relative z-10 w-full bg-background py-16 sm:py-24">
      ...
      <div className="relative mt-10 sm:mt-12">
        ...
        >
            ...
            >
              {platformSurfaceCards.map((card) => (
                <TabsTrigger
                  key={card.id}
                  value={card.id}
                  id={`presence-platform-surface-tab-${card.id}`}
                  aria-controls="presence-platform-surface-panel"
          ...
          <div
            role="tabpanel"
            id="presence-platform-surface-panel"
            aria-labelledby={`presence-platform-surface-tab-${tab}`}
