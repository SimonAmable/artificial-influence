...
export interface SafariProps extends HTMLAttributes<HTMLDivElement> {
  ...
  videoSrc?: string
  mode?: SafariMode
  /** Prefix for SVG `id`s so multiple Safari instances on one page do not clash */
  svgIdPrefix?: string
  /** Alt text for `imageSrc` when present */
  imageAlt?: string
  /** Renders inside the browser viewport (same frame as image/video). Mutually exclusive with image/video in practice. */
  children?: ReactNode
}
...
export function Safari({
  ...
  const hasMedia = hasVideo || !!imageSrc || hasSlot
  const punchId = `${svgIdPrefix}-punch`
  const clipMainId = `${svgIdPrefix}-clip-main`
  const clipRoundedId = `${svgIdPrefix}-clip-rounded`
  ...
  return (
    ...
    >
      ...
      {!hasVideo && !imageSrc && hasSlot && (
        <div
          className="absolute z-0 flex overflow-hidden bg-neutral-950"
          style={{
            left: `${LEFT_PCT}%`,
            top: `${TOP_PCT}%`,
            width: `${WIDTH_PCT}%`,
