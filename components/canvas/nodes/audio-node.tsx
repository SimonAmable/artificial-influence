...
import { INWORLD_TTS_MODEL_OPTIONS } from "@/lib/constants/inworld-tts"
...
export const AudioNodeComponent = React.memo(({ id, data, selected }: NodeProps) => {
  ...
  const provider = nodeData.provider || "inworld"
  ...
  const handleEnhanceScript = async () => {
    ...
    try {
      const response = await fetch("/api/enhance-tts-script", {
        ...
        body: JSON.stringify({
          provider,
          model: nodeData.model || getDefaultAudioModel(provider),
          text: source,
          voice: nodeData.voice,
          stylePrompt: nodeData.stylePrompt || "",
          languageCode:
            nodeData.languageCode || DEFAULT_GOOGLE_GEMINI_LANGUAGE_CODE,
        }),
      })
      ...
      nodeData.onDataChange?.(id, {
        text: enhanced,
        stylePrompt:
          provider === "google" && typeof data.stylePrompt === "string"
            ? data.stylePrompt
            : nodeData.stylePrompt,
      })
  ...
  const handleGenerate = async () => {
    ...
    if (!nodeData.voice.trim()) {
      nodeData.onDataChange?.(id, {
        error:
          provider === "google"
            ? "Choose a Gemini voice first"
            : "Choose an Inworld voice first",
      })
      return
    }
    ...
    try {
      const response = await fetch("/api/generate-audio", {
        ...
        body: JSON.stringify({
          ...
          model: nodeData.model || getDefaultAudioModel(provider),
          stylePrompt:
            provider === "google" ? nodeData.stylePrompt || "" : undefined,
          languageCode:
            provider === "google"
              ? nodeData.languageCode || DEFAULT_GOOGLE_GEMINI_LANGUAGE_CODE
              : undefined,
  ...
  return (
    <>
      <NodeToolbar
        ...
        offset={35}
      >
        <div className="rounded-xl border border-white/10 bg-zinc-900/95 px-2 py-2 shadow-xl backdrop-blur-md flex flex-nowrap items-center gap-2 overflow-x-auto nopan nodrag">
          <ToolbarIconButton
            icon={PaperPlaneTilt}
          ...
          {hasContent ? (
            ...
            <div className="w-full h-full px-4 flex items-center justify-center">
              <p className="text-xs text-zinc-500 text-center leading-relaxed max-w-[220px]">
                {provider === "google"
                  ? "Type script below or connect a Text node, choose a Gemini voice, add style if needed, then generate."
        ...
        <div className="rounded-xl border border-white/10 bg-zinc-900/95 backdrop-blur-md shadow-xl overflow-hidden nopan nodrag">
          <div className="p-2.5 space-y-2">
              ...
              <div className="space-y-2">
                <div className="grid gap-2 sm:grid-cols-[110px_minmax(0,1fr)]">
                  <Input
                    ...
                    className="h-8 border-white/10 bg-zinc-800/60 text-xs text-zinc-100 placeholder:text-zinc-500"
                  />
                  <textarea
                    value={nodeData.stylePrompt || ""}
                    onChange={(event) =>
                      nodeData.onDataChange?.(id, {
                    ...
                    placeholder="Style prompt: describe tone, pace, or delivery..."
                    rows={2}
                    className="w-full resize-none rounded-lg border border-white/10 bg-zinc-800/60 px-2.5 py-2 text-xs text-zinc-200 placeholder:text-zinc-500 outline-none"
                  />
                </div>
                <p className="text-[11px] text-zinc-500">
                  Gemini supports tags like [laughing], [whispering], and [short pause].
                </p>
              </div>
            ) : null}
            ...
            <div className="flex justify-end pt-0.5">
              <Button
                ...
                disabled={isEnhancing || !!nodeData.isGenerating || !fullSpeechText.trim()}
                title={
                  provider === "google"
                    ? "Rewrite script and style prompt for Gemini TTS delivery"
                    : "Rewrite script for natural Inworld TTS (emphasis, pacing, spoken numbers)"
                }
          ...
          <div className="border-t border-white/5 px-3 py-2.5 flex flex-nowrap items-center gap-2 overflow-x-auto">
            <Select
              ...
              onValueChange={(value) =>
                nodeData.onDataChange?.(id, {
                  provider: value as AudioProvider,
                  model: getDefaultAudioModel(value as AudioProvider),
                  voice: "",
                  error: null,
                })
              }
            >
              <SelectTrigger className="h-8 min-w-[130px] shrink-0 bg-zinc-800/80 border-white/10 text-xs">
            ...
            {provider === "inworld" ? (
              <Select
                value={nodeData.model || getDefaultAudioModel("inworld")}
                onValueChange={(value) =>
